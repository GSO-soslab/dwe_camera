...
class CameraDevice:
    ...
    def read_jpeg(self):
        ...
        especially on resource-constrained systems. Since CAP_PROP_CONVERT_RGB is
        false, this should return the raw JPEG data.
        """
        with self.lock:
            if not self.is_opened():
                return None

            # grab() is a lightweight operation to fetch the frame from the camera buffer
            # into an internal OpenCV buffer. This should be fast.
            ret = self.cap.grab()
            if not ret:
                # Use throttling logger to avoid spamming the console
                self.logger.warn("Failed to grab frame from camera.", throttle_duration_sec=5)
                return None

            # retrieve() decodes the frame from the internal buffer.
            # When format is MJPG and CONVERT_RGB is 0, this just moves the raw data.
            ret, frame = self.cap.retrieve()
            if not ret or frame is None:
                self.logger.warn("Failed to retrieve frame after grabbing.", throttle_duration_sec=5)
                return None
            ...
            if frame.ndim == 2:
                ...
                self.logger.error(
                    f"Retrieved frame was not in the expected raw JPEG format. "
